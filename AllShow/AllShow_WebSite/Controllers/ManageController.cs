...
namespace AllShow_WebSite.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            Member member = db.GetMember.Get().Where(m => m.mememail == User.Identity.GetUserName()).FirstOrDefault();
            ViewBag.memid = member.memno;
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            if (member != null)
            {
                member.mempwd = model.NewPassword;
                db.GetMember.Update(member);
        ...
        public ActionResult ChangeInfo()
        {
            ...
            if (member != null)
                ...
                return Content("找不到會員資訊");
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ChangeInfo(Member model, HttpPostedFileBase file)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            try
            {
                if (file != null)
                {
                    ...
                    {
                        if (!Directory.Exists(Server.MapPath("~/images/FileUpLoads/"+model.memno)))
                        {
                            Directory.CreateDirectory(Server.MapPath("~/images/FileUpLoads/" + model.memno));
                        }

                        DateTime d = DateTime.Now;
                        string filename = String.Format("{0:yyyyMMddHHmmss}", d).ToString() + ".jpg";
                        var path = Path.Combine(Server.MapPath("~/images/FileUpLoads/"+ model.memno), filename);
                        file.SaveAs(path);

                        model.mempic = "FileUpLoads/" + model.memno + "/" + filename;
                    }
                }
            }
            catch (Exception ex)
